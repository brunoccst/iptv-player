...
| Module | Purpose |
|--------|---------|
| `tags.py` | Tag vocabularies (quality, source, languages, audio, ignored tokens) and ranking tables. |
| `parser.py` | `parse_title()`: raw title → `ParsedTitle` (clean title, key, year, quality, source, audio, HDR). |
