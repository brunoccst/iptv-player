  ...
  return (
    <View style={styles.screen}>
      ...
      <View style={styles.panel}>
        ...
        <Field label="Server URL" value={serverUrl} onChange={setServerUrl} testID="login-server" autoFocus placeholder="http://provider:8080" />
...
function Field({ label, value, onChange, secure, testID, autoFocus, placeholder }: {
  label: string; value: string; onChange(v: string): void; secure?: boolean; testID: string; autoFocus?: boolean; placeholder?: string;
}) {
  const [focused, setFocused] = useState(false);
  return (
    <View style={styles.field}>
      <Text style={styles.label}>{label}</Text>
      <TextInput testID={testID} accessibilityLabel={label} value={value} onChangeText={onChange} secureTextEntry={secure}
        autoCapitalize="none" autoCorrect={false} placeholder={placeholder} placeholderTextColor={colors.muted}
        hasTVPreferredFocus={autoFocus} onFocus={() => setFocused(true)} onBlur={() => setFocused(false)}
...
const styles = StyleSheet.create({
  ...
  brand: { position: 'absolute', top: safe.vertical, left: safe.horizontal, color: colors.accent, fontSize: fonts.title, fontWeight: '900' },
  ...
  input: { backgroundColor: '#333', color: colors.strong, fontSize: fonts.body, paddingHorizontal: spacing.md, paddingVertical: spacing.sm, borderRadius: 4, borderWidth: 2, borderColor: 'transparent' },
