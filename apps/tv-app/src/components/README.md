...
| `AccountMenu.tsx` | Menu under the avatar: other profiles, Manage Profiles, Parental PIN, Sync with phone (TV) or Connect a TV (phone, D-060), Back up data, Playback (when FFmpeg is bundled), Refresh library, Check for updates (release builds, D-062), Log, Sign out. `confirmSignOut` asks first. |
