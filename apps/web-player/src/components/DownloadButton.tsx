...
export function DownloadButton({ target }: { target: DownloadTarget }) {
  ...
  const label =
    status === 'completed' ? `Downloaded: ${target.title}`
    : status === 'downloading' || status === 'queued' ? `Pause download (${Math.round(progress * 100)}%)`
    : status === 'paused' ? `Resume download (${Math.round(progress * 100)}%)`
    : status === 'error' ? `Download failed: ${record?.error ?? ''}. Retry`
  ...
  return (
    <button type="button" className={`download-button${status ? ` download-button--${status}` : ''}`} onClick={onClick}
      aria-label={label} title={label} disabled={status === 'completed'}>
      {status === 'downloading' || status === 'queued' || status === 'paused' ? <ProgressRing value={progress} /> : null}
      <Icon
        name={status === 'completed' ? 'check' : status === 'error' ? 'alert' : status === 'downloading' || status === 'queued' ? 'pause' : 'download'}
