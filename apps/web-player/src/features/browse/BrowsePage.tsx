  ...
  return (
    <div className="page">
      ...
      <div className="chips" role="tablist" aria-label="Categories">
        <button type="button" role="tab" aria-selected={categoryId === null} className={`chip${categoryId === null ? ' chip--active' : ''}`}
          onClick={() => setCategoryId(null)}>All</button>
        {categories.map((category) => (
          <button key={category.id} type="button" role="tab" aria-selected={categoryId === category.id}
            className={`chip${categoryId === category.id ? ' chip--active' : ''}`} onClick={() => setCategoryId(category.id)}>
            {category.name}
          </button>
        ))}
      </div>
...
export function PagedGrid({ section, categoryId = null, search = null }: { section: LibrarySection; categoryId?: string | null; search?: string | null }) {
  ...
  if (last?.status === 'error') return <p className="error-text" role="alert">{errorText(last.error)}</p>;
    ...
    <>
      ...
      {items.length < total ? (
        <div style={{ textAlign: 'center', marginTop: 24 }}>
          <button type="button" className="button button--secondary" disabled={last?.status === 'loading'} onClick={() => setPages(pages + 1)}>
            Load more
          </button>
        </div>
      ) : null}
