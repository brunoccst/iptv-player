...
export function DetailsModal({ target }: { target: DetailsTarget }) {
  ...
  return (
    <Modal label={resource?.data?.title ?? 'Details'} onClose={close}>
      {resource?.data ? (
        target.section === 'movies' ? <MovieDetails master={resource.data} /> : <SeriesDetailsView master={resource.data} />
      ) : resource?.status === 'error' ? (
        <p className="error-text" role="alert" style={{ padding: 32 }}>{errorText(resource.error)}</p>
      ) : (
        <div style={{ padding: 64, display: 'grid', placeItems: 'center' }}><Spinner /></div>
      )}
    </Modal>
  );
}
...
function useMasterProgress(masterId: string, variants: VariantInfo[], kind: 'movie' | 'episode'): ProgressDto | null {
  return useProgress((s) =>
    (s.items.data ?? []).find((p) => p.kind === kind && (p.masterId === masterId || variants.some((v) => v.streamId === (kind === 'movie' ? p.itemId : p.seriesId)))) ?? null);
}
...
function MovieDetails({ master }: { master: MasterDetails }) {
  ...
  useEffect(() => {
    ...
    if (resume && master.variants.some((v) => v.streamId === resume.itemId)) stores.library.getState().selectVariant(master.id, resume.itemId);
    ...
    <>
      ...
      <div className="details__body">
        ...
        <div className="details__side">
          {meta.data?.cast ? <p>Cast: <strong>{meta.data.cast}</strong></p> : null}
          {meta.data?.genre ? <p>Genres: <strong>{meta.data.genre}</strong></p> : null}
          {meta.data?.director ? <p>Director: <strong>{meta.data.director}</strong></p> : null}
          <p>Source: <strong>{variant.rawTitle}</strong></p>
        </div>
      </div>
    </>
  );
  ...
  const play = () => {
    if (resume && resume.seriesId === variant.streamId) uiStore.getState().play(progressTarget(resume));
      ...
      uiStore.getState().play({
        kind: 'episode', streamId: firstEpisode.id, container: firstEpisode.containerExtension, title: master.title,
        subtitle: firstEpisode.title, posterUrl: master.posterUrl, masterId: master.id, seriesId: variant.streamId,
        seasonNumber: firstEpisode.seasonNumber, episodeNumber: firstEpisode.episodeNumber,
      });
    }
  };
    ...
    <>
      <DetailsHero backdrop={series.data?.backdropUrls[0] ?? master.posterUrl} title={master.title}>
        <button type="button" className="button button--primary" onClick={play} disabled={!series.data}>
          <Icon name="play" /> {resume && resume.seriesId === variant.streamId ? `Resume S${resume.seasonNumber}:E${resume.episodeNumber}` : 'Play'}
        </button>
      </DetailsHero>
      <div className="details__body">
        <div>
          ...
          <VariantSelect variants={master.variants} value={variant.streamId}
            onChange={(streamId) => stores.library.getState().selectVariant(master.id, streamId)} />
        </div>
        <div className="details__side">
          {series.data?.cast ? <p>Cast: <strong>{series.data.cast}</strong></p> : null}
          {series.data?.summary.genre ? <p>Genres: <strong>{series.data.summary.genre}</strong></p> : null}
        </div>
      </div>
      {series.loading ? <div style={{ padding: 32 }}><Spinner /></div> : null}
      {series.error ? <p className="error-text" style={{ padding: '0 32px 32px' }}>{errorText(series.error)}</p> : null}
      {series.data ? (
        <EpisodeList key={variant.streamId} series={series.data} title={master.title} masterId={master.id} seriesId={variant.streamId}
...
function Facts({ year, rating, quality, runtime, extra }: {
  year: number | null; rating: number | null | undefined; quality: string | null; runtime?: number | null; extra?: string | null;
}) {
  return (
