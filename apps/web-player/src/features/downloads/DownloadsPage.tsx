...
export function DownloadsPage() {
  ...
  return (
    <div className="page">
      ...
      {estimate ? (
        <p className="muted">Using {formatBytes(estimate.usage)} of {formatBytes(estimate.quota)} available to this app.</p>
      ) : null}
      ...
      <div className="downloads__list">
        {records.map((record) => <DownloadItem key={record.id} record={record} />)}
      </div>
    </div>
  );
}
...
function DownloadItem({ record }: { record: DownloadRecord }) {
  ...
  return (
    <article className="download-item" aria-label={record.title}>
      {record.posterUrl ? <img src={posterPath(record.id)} alt="" onError={(e) => (e.currentTarget.src = record.posterUrl!)} /> : <span className="download-item__art" />}
      <div>
        <p className="download-item__title">{record.title}</p>
        {record.subtitle ? <p className="muted" style={{ margin: 0 }}>{record.subtitle}</p> : null}
        <p className={record.status === 'error' ? 'error-text' : 'muted'} style={{ margin: '6px 0 0' }}>{statusText}</p>
        {record.status !== 'completed' ? <div className="download-item__bar"><span style={{ width: `${progress * 100}%` }} /></div> : null}
      </div>
      <div className="download-item__actions">
        {record.status === 'completed' ? (
          <button type="button" className="icon-button" aria-label={`Play ${record.title}`} onClick={() => uiStore.getState().play(playTargetFromDownload(record))}>
